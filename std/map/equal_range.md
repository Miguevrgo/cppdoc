...
1, 2. Returns a pair of iterators `{lower_bound(key), upper_bound(key)}`, delimiting the range of elements with key equivalent to `key`. Since map keys are unique, the range holds at most one element.
