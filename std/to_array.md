...
2. Moves the elements of the rvalue C-style array `a` into a new `std::array`. `T` must be copy- or move-constructible as needed, and not an array of unknown bound.
...
auto arr2 = std::to_array<int>({1, 2, 3, 4}); // (2) elements moved from a temporary
assert((arr2 == std::array{1, 2, 3, 4}));
```
