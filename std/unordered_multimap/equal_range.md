---
symbol: std::unordered_multimap::equal_range
...
1, 2. Returns a pair of iterators delimiting the range `[first, last)` of all elements with key equivalent to `key`. If there are no such elements, both iterators are equal and point to a location where such an element would be inserted, but not necessarily `end()`. All elements with the same key are guaranteed to be adjacent.
...
std::unordered_multimap<int, int> numbers{{1, 10}, {3, 30}, {3, 31}, {5, 50}};

auto [first, last] = numbers.equal_range(3);
assert(std::distance(first, last) == 2);
for (auto it = first; it != last; ++it) assert(it->first == 3);
```
